...
// This version sends one genome at a time to the available helpers. If
// the calculation time is relatively large and variable, this can be
// advantageous.
	...
	else
	{
		...
		else // we're a helper: the only things we should receive are fitness calculation results and factory feedback
		{
			if (cmd == GASERVER_COMMAND_FITNESS) 
			{
				...
				{
					writeLog(LOG_DEBUG, "processPacketWhenRunning: invalid number of genome results %d", numGenomes);
					return false;
				}
...
{
	//writeLog(LOG_DEBUG, "sendData: m_state = %d", (int)m_state);
	...
	m_lastWrittenGenomeNumbers.clear();
	while (count < 1 && (genomeNum = pAlg->getGenomeToProcess()) >= 0) // only send at most one genome
	{
		count++;
		m_lastWrittenGenomeNumbers.push_back(genomeNum);
	}

	//writeLog(LOG_DEBUG, "sendData: sending count = %d genomes", count);
...
bool ServerAlgorithm::calculateFitness(std::vector<GenomeWrapper> &population)
{
	...
	writeLog(LOG_DEBUG, "Current connection info:");
...
//	double startTime = getCurrentTime();
	...
	while (m_genomesCalculated != populationSize && m_pClientConnection && !endServerLoop)
	{
		...
		if (m_pClientConnection)
		{
			...
			//writeLog(LOG_DEBUG, "calculateFitness: sending outgoing packets");
				...
				for (it = m_connections.begin() ; waitTimeSec != 0 && it != m_connections.end() ; it++)
				{
					...
					if (pInf != m_pClientConnection)
					{
						if (pInf->getConnectionState() == ConnectionInfo::Idle && pInf->canHelp())
						{
							...
							writeLog(LOG_DEBUG, "calculateFitness: found client which is idle");
						}
					}
				}
			}
			
			//writeLog(LOG_DEBUG, "calculateFitness: end sending outgoing packets");
		}
	}
